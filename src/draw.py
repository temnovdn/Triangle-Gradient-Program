...
red_x_area = Frame(master=red_coordinates)
red_x_area.pack(side=TOP)

red_y_area = Frame(master=red_coordinates)
red_y_area.pack(side=TOP)
...
red_y_input.pack(side=LEFT)

green_x_input = Entry(master=green_coordinates, bg='green', fg='white', width=10)
green_x_input.insert(END, '%s-%s' % (0, draw_area_width))
